...
// getAll genérico no hace populate, así que creamos uno custom:
export const listarObras = async (req, res) => {
    try {
      const data = await Obra.find({ user: req.user.id })
        .populate("cliente", "nombre apellido"); // solo campos nombre, apellido
      res.json(data);
    } catch (error) {
      res
        .status(500)
